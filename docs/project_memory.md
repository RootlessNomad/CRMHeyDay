...
- **Last Completed**: **UJ-34** — Lead Discovery por Zona con Email Personalizado. Pestaña "Buscar" en /intel/research (tab por defecto). Backend: queue `lead_discovery` + módulo `modules/lead-discovery/` (Places → score ≥ 40 → Company + Lead + OutboundPrep con `emailDraft`). Prompt Claude `leadDiscoveryEmailPrompt` genera score + briefing + email en una sola llamada. Frontend: `LeadDiscoveryForm` + `LeadDiscoveryStatus` + sección "Email en frío" con copiar en `OutboundPrepCard`. Typecheck + lint ✅. Migración `add_email_draft` pendiente de aplicar con Docker.
...
- **Next Step**: **Pasos operativos del usuario** (no-código): (1) Docker arriba → aplicar migraciones pendientes (`add_email_draft`, `add_demo_link`, deuda UJ-28/29) con `prisma migrate deploy`; (2) key `google_places` cargada en vault (ya en prod); (3) probar UJ-34 en local: /intel/research → Buscar → ciudad + tipo; (4) deploy a EasyPanel con el nuevo endpoint `/intel/lead-discovery`; (5) DNS `demos.estudioheyday.com` si aún pendiente.
